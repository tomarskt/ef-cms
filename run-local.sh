...
echo "starting irs case documents service"
npx sls offline start "$@" --config web-api/serverless-irs-case-documents.yml &
echo "starting irs cases service"
npx sls offline start "$@" --config web-api/serverless-irs-cases.yml &
